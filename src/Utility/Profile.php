...
class Profile
{
     ...
     * @var array
     */
    private static $currentProfileNames = [];
     ...
     * @var array
     */
    private static $profileNames = [];

    /**
     * @param string $realName
     * @return string
     */
    private static function getName($realName)
    {
     ...
     * @return array
     */
    private static function getNestingProfiles()
    {
        $profiles = [];
        $dx = strlen(self::PATH_SEPARATOR);
        foreach (self::$profiles as $path => $profile) {
            $profile['title'] = self::getName(
                strrpos($path, self::PATH_SEPARATOR) !== false ? substr(
                    $path,
                    strrpos($path, self::PATH_SEPARATOR) + $dx
                ) : $path
            );
            $profile['children'] = [];
            eval(
     ...
     * @return string
     */
    private static function getResultHtml($profiles, $level = 0)
    {
        $html = '';
        $now = microtime(true);
        ...
        foreach ($profiles as $profile) {
            ...
            if (!empty($profile['children'])) {
                $html .= sprintf(
                    '<tr><td class="name">%s<span>%s</span> start</td><td class="time">%s</td><td class="memory">%s</td></tr>',
                    $spaces,
                    $profile['title'],
                    '-',
                    '-'
                    ...
                    sprintf(
                        '<tr><td class="name">%s<span>%s</span> end</td><td class="time">%s</td><td class="memory">%s</td></tr>',
                        $spaces,
                        $profile['title'],
                        $usedTime,
                        number_format($usedMemory, 0, '.', ', ')
                    );
            } else {
                $html .= sprintf(
                    '<tr><td class="name">%s<span>%s</span></td><td class="time">%s</td><td class="memory">%s</td></tr>',
                    $spaces,
                    $profile['title'],
                    $usedTime,
                    number_format($usedMemory, 0, '.', ', ')
    ...
    public static function printProfiles()
    {
        ...
        echo sprintf(
            '<table class="profiles"><thead><tr>' .
            '<th class="name">Profile Name</th>' .
            '<th class="time">Used Time (ms)</th>' .
            '<th class="memory">Used Memory (byte)</th></tr></thead>' .
            '<tbody>%s</tbody><tfoot><tr><td colspan="3">&nbsp;</td></tr></tfoot></table>',
            self::getResultHtml(self::getNestingProfiles())
        );
    }
